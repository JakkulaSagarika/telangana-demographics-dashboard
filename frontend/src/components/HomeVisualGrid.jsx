...
export default function HomeVisualGrid({ districts, overview }) {
  ...
  return <>
    ...
    <section className="home-bottom"><Panel eyebrow="Recent updates" title="Registration highlights"><div className="update-list"><Link to={`/demographics/districts/${birthLeader?.slug}`}><span>Highest birth count</span><b>{birthLeader?.name || '—'}</b><strong>{format(birthLeader?.births)}</strong> →</Link><Link to={`/demographics/districts/${deathLeader?.slug}`}><span>Highest death count</span><b>{deathLeader?.name || '—'}</b><strong>{format(deathLeader?.deaths)}</strong> →</Link></div></Panel><Panel eyebrow="Quick insights" title="Statewide snapshot"><div className="insight-list"><div><span>Female share of births</span><b>{femaleShare}%</b></div><div><span>Births per recorded death</span><b>{overview.deaths ? (overview.births / overview.deaths).toFixed(2) : '—'} : 1</b></div><div><span>Districts with imported records</span><b>{overview.district_count || 0}</b></div></div></Panel></section>
  </>
}
