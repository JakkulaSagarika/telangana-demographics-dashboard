...
export default function InsightStudio({ districts, overview }) {
  ...
  return <section className="insight-studio">
    ...
    {selected && <Link className="selection-chip" to={`/districts/${selected.slug}`}><span>Selected district</span><b>{selected.name}</b><strong>{number(selected[metric])}</strong> Open profile →</Link>}
