...
export default function DistrictTable() {
  ...
  <div className="table-wrap"><table><thead><tr>{columns.map(([key, label]) => <th key={key}><button onClick={() => setSort(key)}>{label} ↕</button></th>)}</tr></thead><tbody>{rows.map(d => <tr key={d.slug}>{columns.map(([key]) => <td key={key}>{key === 'name' ? <Link to={`/districts/${d.slug}`}>{d.name}</Link> : format.format(d[key])}</td>)}</tr>)}</tbody></table>{!rows.length && <div className="empty-copy">No district data has been imported yet.</div>}</div></>
}
